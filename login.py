...
def login():
    ...
    if user_login_attempts <= 2 and name in users:
        ...
        while account_password != password and password_login_attempts <= 1:
            password_login_attempts = password_login_attempts + 1
            print('Uw wachtwoord is onjuist. Probeer het nogmaals.')
            password = str(input('Wachtwoord: '))

    if name in users and account_password == password:
